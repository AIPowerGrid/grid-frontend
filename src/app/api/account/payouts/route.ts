...
/** The signed-in account's custodial payout history — paid (with tx hashes)
 * and accrued balances (proxies /v1/account/payouts). */
export async function GET(req: NextRequest) {
  ...
  const res = await fetch(`${GRID_API_BASE}/v1/account/payouts`, {
    headers: { apikey: key },
    cache: 'no-store'
  });
